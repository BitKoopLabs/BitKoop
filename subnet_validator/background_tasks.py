...
def start_set_weights_thread(context: AppContext) -> tuple[threading.Thread | None, threading.Event | None]:
    """Start background thread for periodically setting weights."""
    stop_event = threading.Event()
    thread = threading.Thread(target=run_set_weights_loop, args=(context, stop_event), daemon=True)
    thread.start()
    logger.info("Started set_weights worker thread")
    return thread, stop_event


def run_validate_coupons_loop(context: AppContext, stop_event: threading.Event):
    """Run periodic validate_coupons in its own thread using an asyncio loop."""
    async def _worker():
        from .database.database import get_db
        from .tasks.validate_coupons import validate_pending_coupons, validate_outdated_coupon
...
def start_sync_sites_thread(context: AppContext) -> tuple[threading.Thread | None, threading.Event | None]:
    """Start background thread for periodically syncing sites."""
    stop_event = threading.Event()
    thread = threading.Thread(target=run_sync_sites_loop, args=(context, stop_event), daemon=True)
    thread.start()
    logger.info("Started sync_sites worker thread")
    return thread, stop_event


def start_validate_coupons_thread(context: AppContext) -> tuple[threading.Thread | None, threading.Event | None]:
    """Start background thread for periodically validating coupons."""
    stop_event = threading.Event()
    thread = threading.Thread(target=run_validate_coupons_loop, args=(context, stop_event), daemon=True)
    thread.start()
