...
from subnet_validator.services.validator.api_coupon_validator import ApiCouponValidator
...
from subnet_validator.services.validator.playwright_coupon_validator import PlaywrightCouponValidator
from subnet_validator.services.validator.tlsn_coupon_validator import TlsnCouponValidator
...
def get_coupon_validator(site: Site, settings: Annotated[Settings, Depends(get_settings)]) -> BaseCouponValidator:
    try:
        if site.config and isinstance(site.config, dict) and site.config.get("validation") == "tlsn":
            return TlsnCouponValidator(site=site, verifier_url=settings.tlsn_verifier_url)
        if site.api_url:
